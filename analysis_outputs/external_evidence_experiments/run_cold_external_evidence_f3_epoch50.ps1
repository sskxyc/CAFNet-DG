...
& $python cold-scence.py `
  ...
  --max_folds 3 `
  --tenfold `
  --result_prefix cd3ext_atc_f3e50 `
  --short_result_name `
  --lr 0.0004 `
  --wd 0.001 `
  --lamb 0.03 `
  ...
  --drug_feature_file data_external\chembl_atc_features.npy `
  --drug_feature_tag atc `
  --evidence_dropout 0.1 `
  > (Join-Path $logDir "cd3ext_atc_f3e50_stdout.log") `
  2> (Join-Path $logDir "cd3ext_atc_f3e50_stderr.log")
