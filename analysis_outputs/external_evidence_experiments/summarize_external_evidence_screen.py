...
SCREEN_DIRS = {
    "CAFNet-D + ATC": ROOT / "result_ICS" / "10cd3ext_atc_f3e50_CAFNetDecoupled",
    "CAFNet-D + ATC+target": ROOT / "result_ICS" / "10cd3ext_atctarget_f3e50_CAFNetDecoupled",
}
...
def load_scores(selected_rows, train_rows, fold_of_local):
    scores = {
...
def macro_and_subgroups(y_true, scores, R, train_rows, fold_of_local):
    ...
    return pd.DataFrame(macro_rows), pd.DataFrame(subgroup_rows)
...
def summarize_result_csv():
    ...
    for name, d in SCREEN_DIRS.items():
        ...
        avg_rows = np.where(df["pearson"].astype(str).str.lower() == "avg")[0]
        if len(avg_rows):
            df = df.iloc[: int(avg_rows[0])].copy()
        ...
        rows.append(row)
    pd.DataFrame(rows).to_csv(OUT / "external_evidence_result_csv_summary.csv", index=False)
...
def main():
    ...
    y_true, scores, R = load_scores(selected_rows, train_rows, fold_of_local)
    ...
    macro.to_csv(OUT / "external_evidence_macro_by_drug.csv", index=False)
    subgroup.to_csv(OUT / "external_evidence_subgroup_by_drug.csv", index=False)
    matched.to_csv(OUT / "external_evidence_matched_by_drug.csv", index=False)
    macro_s = summarize(macro, ["model"], ["AP", "nDCG@10", "P@15", "R@15"])
    subgroup_s = summarize(subgroup, ["group", "model"], ["AP", "nDCG@10", "P@15", "R@15"])
    matched_s = summarize(matched, ["model"], ["matched_AUROC", "matched_AUPR", "pos_gt_ctrl_rate", "pos_minus_ctrl_score"])
    macro_s.to_csv(OUT / "external_evidence_macro_summary.csv", index=False)
    subgroup_s.to_csv(OUT / "external_evidence_subgroup_summary.csv", index=False)
    matched_s.to_csv(OUT / "external_evidence_matched_summary.csv", index=False)
    ...
    report.to_csv(OUT / "external_evidence_screen_report.csv", index=False)
