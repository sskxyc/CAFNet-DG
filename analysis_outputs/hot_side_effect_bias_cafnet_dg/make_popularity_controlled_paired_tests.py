...
OUT = ROOT / "analysis_outputs" / "hot_side_effect_bias_cafnet_dg"
INFILE = OUT / "removed_hot_side_effect_metrics_by_fold.csv"

TARGET = "CAFNet-DG"
BASELINES = ["CAFNet", "Decoupled-v3", "Global popularity", "HSTrans"]
...
def main():
    ...
    for (removed, baseline), sub in paired.groupby(["removed_top_n", "baseline"], sort=False):
        wins = sub[(sub["delta_mean"] > 0) & sub["wilcoxon_holm_sig"]]["metric"].tolist()
        numeric = sub[(sub["delta_mean"] > 0) & (~sub["wilcoxon_holm_sig"])]["metric"].tolist()
        losses = sub[(sub["delta_mean"] < 0) & sub["wilcoxon_holm_sig"]]["metric"].tolist()
        summary_rows.append(
            {
                "removed_top_n": removed,
                "baseline": baseline,
    ...
    lines = [
        ...
        "\\caption{Popularity-controlled cold-start paired tests after removing the top-$N$ most frequent side effects. Values are mean $\\pm$ standard deviation over 10 folds. Positive $\\Delta$ indicates better CAFNet-DG performance.}\\label{tab:popularity_controlled_dg_paired}\\\\",
        "\\toprule",
        "Removed & Baseline & Metric & Baseline & CAFNet-DG & $\\Delta$ & $p_t^{Holm}$ & $p_w^{Holm}$ \\\\",
        "\\midrule",
        "\\endfirsthead",
        "\\toprule",
        "Removed & Baseline & Metric & Baseline & CAFNet-DG & $\\Delta$ & $p_t^{Holm}$ & $p_w^{Holm}$ \\\\",
    ...
    for _, row in paired.iterrows():
        marker = "$^{\\dagger}$" if row["wilcoxon_holm_sig"] and row["delta_mean"] > 0 else ""
        loss = "$^{\\ddagger}$" if row["wilcoxon_holm_sig"] and row["delta_mean"] < 0 else ""
        lines.append(
    ...
    lines += [
        ...
        "\\end{longtable}",
        "",
        "\\noindent $\\dagger$ indicates a Holm-corrected Wilcoxon signed-rank improvement for CAFNet-DG; $\\ddagger$ indicates a significant loss.",
        "",
    ]
