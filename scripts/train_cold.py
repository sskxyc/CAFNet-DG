...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run CAFNet-DG cold-start training/evaluation from a JSON config.")
    parser.add_argument(
        "--config",
        type=Path,
        default=Path(__file__).resolve().parents[1] / "configs" / "cafnet_d_cold_main.json",
    ...
    parser.add_argument("extra", nargs=argparse.REMAINDER, help="Extra arguments passed to cold-scence.py.")
    args = parser.parse_args()

    cmd = _build_command(args.config, args.seed, args.result_prefix, args.extra)
    print(f"[train_cold] command: {' '.join(cmd)}")
