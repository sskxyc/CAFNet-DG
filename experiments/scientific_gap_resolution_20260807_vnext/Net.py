...
class CAFNetDecoupled(CAFNetV2):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.rank_fusion_mode = str(rank_fusion_mode)
        ...
        self.register_buffer("side_tailness", torch.zeros(num_sides, dtype=torch.float32))
        self.register_buffer("global_mean", torch.zeros(1, dtype=torch.float32))
        pair_dim = output_dim * 4 + 1
        self.tail_head = None
        self.global_gate_logit = None
        self.drug_rank_gate = None
        self.pair_rank_gate = None
        self.tail_gate_scale = None
        prior_logit = math.log(self.gate_prior / max(1e-6, 1.0 - self.gate_prior))
        if self.rank_fusion_mode != "none":
        ...
        if self.rank_fusion_mode == "global":
            self.global_gate_logit = nn.Parameter(torch.tensor(prior_logit, dtype=torch.float32))
        elif self.rank_fusion_mode in {"drug", "drug_stratum"}:
            self.drug_rank_gate = nn.Linear(output_dim, 1)
            nn.init.zeros_(self.drug_rank_gate.weight)
            nn.init.constant_(self.drug_rank_gate.bias, prior_logit)
            if self.rank_fusion_mode == "drug_stratum":
                self.tail_gate_scale = nn.Parameter(torch.tensor(-1.0, dtype=torch.float32))
        elif self.rank_fusion_mode == "pair":
            self.pair_rank_gate = nn.Sequential(
                nn.Linear(pair_dim, output_dim // 2),
                nn.ReLU(),
                nn.Linear(output_dim // 2, 1),
            )
            nn.init.zeros_(self.pair_rank_gate[-1].weight)
            nn.init.constant_(self.pair_rank_gate[-1].bias, prior_logit)
        elif self.rank_fusion_mode != "none":
            raise ValueError("Unknown rank_fusion_mode: {}".format(self.rank_fusion_mode))
        self.last_gate = None
        self.last_tail_score = None
        self.last_head_score = None
    ...
    def set_frequency_priors(self, side_popularity=None, global_mean=0.0):
        if side_popularity is not None:
            ...
            ranks = torch.argsort(torch.argsort(side_pop))
            tailness = 1.0 - ranks.to(dtype=torch.float32) / max(1, side_pop.numel() - 1)
            self.side_tailness.copy_(tailness)
            side_pop_z = (side_pop - side_pop.mean()) / side_pop.std().clamp_min(1e-6)
            self.side_popularity.copy_(side_pop_z)
    ...
    def forward(self, data, data_e, DF=False, not_FC=True, alpha=0.15):
        ...
        self.last_head_score = rank_score
        self.last_gate = None
        self.last_tail_score = None
        if self.rank_fusion_mode != "none":
            tail_score = base_score + self.tail_head(pair).squeeze(-1)
            if self.rank_fusion_mode == "global":
                gate = torch.sigmoid(self.global_gate_logit).view(1, 1).expand_as(rank_score)
            elif self.rank_fusion_mode == "drug":
                gate = torch.sigmoid(self.drug_rank_gate(drug_embed)).expand_as(rank_score)
            elif self.rank_fusion_mode == "drug_stratum":
                drug_logit = self.drug_rank_gate(drug_embed)
                tail_offset = self.tail_gate_scale * (self.side_tailness.view(1, -1) - 0.5)
                gate = torch.sigmoid(drug_logit + tail_offset)
            else:
                gate = torch.sigmoid(self.pair_rank_gate(pair).squeeze(-1))
            rank_score = gate * rank_score + (1.0 - gate) * tail_score
            self.last_gate = gate
