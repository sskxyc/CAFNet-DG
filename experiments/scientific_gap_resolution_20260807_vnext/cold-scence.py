...
def prevalence_group_rank_loss(logits, labels, side_prevalence, samples_per_row=32):
    """Average pairwise ranking loss over equal-size prevalence tertiles."""
    if side_prevalence is None:
        return logits.new_tensor(0.0)
    prevalence = side_prevalence.to(device=logits.device, dtype=logits.dtype)
    ordered = torch.argsort(prevalence)
    groups = torch.tensor_split(ordered, 3)
    losses = []
    for cols in groups:
        if cols.numel() == 0:
            continue
        losses.append(bpr_loss(logits[:, cols], labels[:, cols], samples_per_row=samples_per_row))
...
def cafnet_v2_loss(model, output, label, lam, eps, assoc_weight, freq_weight, rank_weight, bpr_samples,
                   ...
                   external_target="rank", group_rank_weight=0.0,
                   gate_reg_weight=0.0, gate_prior=0.6):
    ...
    if group_rank_weight > 0:
        loss = loss + float(group_rank_weight) * prevalence_group_rank_loss(
            rank_source, label, side_prevalence, samples_per_row=bpr_samples
        )
    gate = getattr(model, "last_gate", None)
    if gate_reg_weight > 0 and gate is not None:
        gate_safe = gate.clamp(1e-6, 1.0 - 1e-6)
        prior = gate_safe.new_tensor(float(gate_prior)).clamp(1e-6, 1.0 - 1e-6)
        gate_kl = gate_safe * torch.log(gate_safe / prior) + \
            (1.0 - gate_safe) * torch.log((1.0 - gate_safe) / (1.0 - prior))
        loss = loss + float(gate_reg_weight) * gate_kl.mean()
...
def train(model, device, train_loader, optimizer, lamb, epoch, log_interval, sideEffectsGraph, raw, id, DF, not_FC, eps,
          ...
          external_samples_per_drug=64, external_target="rank", group_rank_weight=0.0,
          gate_reg_weight=0.0, gate_prior=0.6):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        if dual_task:
            ...
            loss = cafnet_v2_loss(
                ...
                group_rank_weight=group_rank_weight,
...
def evaluate(model, device, loader, sideEffectsGraph, DF, not_FC, result_folder, id):
    ...
    with torch.no_grad():
        ...
        for data in loader:
            ...
            gate = getattr(model, "last_gate", None)
            if gate is not None:
                total_gates = torch.cat((total_gates, gate.detach().cpu()), 0)
    ...
    if total_gates.numel() > 0:
        gate_np = total_gates.numpy()
        tailness = model.side_tailness.detach().cpu().numpy()
        ordered = np.argsort(tailness)[::-1]
        rare, middle, frequent = np.array_split(ordered, 3)
        route_file = os.path.join(result_folder, 'routing_diagnostics.csv')
        route_row = {
            'fold': id - 1,
            'gate_mean': float(gate_np.mean()),
            'gate_std': float(gate_np.std()),
            'gate_fraction_below_005': float(np.mean(gate_np < 0.05)),
            'gate_fraction_above_095': float(np.mean(gate_np > 0.95)),
            'gate_rare_mean': float(gate_np[:, rare].mean()),
            'gate_middle_mean': float(gate_np[:, middle].mean()),
            'gate_frequent_mean': float(gate_np[:, frequent].mean()),
        }
        pd.DataFrame([route_row]).to_csv(
            route_file, mode='a', header=not os.path.exists(route_file), index=False
        )
...
def main(modeling, metric, train_batch, lr, num_epoch, knn, weight_decay, lamb, log_interval, cuda_name, frequencyMat,
         ...
         group_rank_weight=0.0, rank_fusion_mode="none", gate_prior=0.6,
         gate_reg_weight=0.0):
    print('\n=======================================================================================')
    print('\n第 {} 次训练：\n'.format(id))
    ...
    if modeling.__name__ == "CAFNetDecoupled":
        ...
        print('group_rank_weight: ', group_rank_weight)
        print('rank_fusion_mode: ', rank_fusion_mode)
        print('gate_prior: ', gate_prior)
        print('gate_reg_weight: ', gate_reg_weight)
    if side_feature_file:
    ...
    if matched_bpr or group_rank_weight > 0:
    ...
    if modeling.__name__ in ["CAFNet", "CAFNetV2", "CAFNetDecoupled"]:
        model = modeling(
            ...
            **({"pop_weight": pop_weight, "bias_weight": bias_weight,
                ...
                "rank_fusion_mode": rank_fusion_mode, "gate_prior": gate_prior}
    ...
    # Do not create an implicit root-level checkpoint during audit runs.
...
if __name__ == '__main__':
    ...
    parser.add_argument('--group_rank_weight', type=float, required=False, default=0.0,
                        help='Weight for equalized rare/middle/frequent prevalence-group BPR')
    parser.add_argument('--rank_fusion_mode', type=str, required=False, default='none',
                        choices=['none', 'global', 'drug', 'drug_stratum', 'pair'],
                        help='End-to-end fusion of the head and tail-aware ranking experts')
    parser.add_argument('--gate_prior', type=float, required=False, default=0.6,
                        help='Initialization and regularization prior for the head-expert gate')
    parser.add_argument('--gate_reg_weight', type=float, required=False, default=0.0,
                        help='Bernoulli KL regularization weight for the learned fusion gate')
    ...
    group_rank_weight = args.group_rank_weight
    rank_fusion_mode = args.rank_fusion_mode
    ...
    if modeling.__name__ in ["CAFNet", "CAFNetV2", "CAFNetDecoupled"] and (
        ...
        if modeling.__name__ == "CAFNetDecoupled":
            ...
            if group_rank_weight > 0:
                cafnet_tag += '_grw=' + str(group_rank_weight)
            if rank_fusion_mode != 'none':
                cafnet_tag += '_rfm=' + str(rank_fusion_mode) + '_gp=' + str(gate_prior) + \
                              '_gr=' + str(gate_reg_weight)
    if side_feature_file:
    ...
    for (id, frequencyMat, mask) in split_data(tenfold, mask_file=mask_file, max_folds=max_folds,
        ...
        main(modeling, metric, train_batch, lr, num_epoch, knn, weight_decay, lamb, log_interval, cuda_name,
             ...
             group_rank_weight, rank_fusion_mode, gate_prior, gate_reg_weight)
